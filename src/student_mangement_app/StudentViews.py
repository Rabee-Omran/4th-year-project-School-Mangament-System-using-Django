...
from student_mangement_app.models import Attendance, AttendanceProgram, AttendanceReport, ContactStudent, Courses, CustomUser, FeedBackStudent, LeaveReportStudent, SessionYearModel, StudentResult, Students, Subjects
import datetime
...
def student_view_attendance_post(request):
    ...
    start_data_parse=datetime.datetime.strptime(start_date,"%Y-%m-%d").date()  # parse to python can understand the date
...
def manager_msg_student(request):
    stuudent_obj = Students.objects.get(admin_id=request.user.id)
    manager_msg = ContactStudent.objects.filter(student_id= stuudent_obj)
  
    return render(request,"student_template/manager_contact.html",{"messages":manager_msg})


def att_program_s(request):


    student = Students.objects.get(admin=request.user.id)
    try:
        att = AttendanceProgram.objects.get(course_id= student.course_id)
    except:
        att = None


    return render(request,"student_template/attendance_program.html",{'att':att})
